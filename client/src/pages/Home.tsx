  ...
  return (
    <article className={`group rounded-[22px] border border-slate-200/80 bg-white p-4 shadow-[0_10px_30px_rgba(15,23,42,0.04)] transition-all duration-200 hover:-translate-y-1 hover:shadow-[0_18px_38px_rgba(15,23,42,0.09)] ${compact ? "" : "sm:p-5"}`}>
      ...
      <div className="mt-4 flex gap-2">
        ...
        <button onClick={copyLink} className="rounded-xl border border-slate-200 px-3 py-2.5 text-slate-500 transition-colors hover:border-slate-300 hover:text-slate-900"><Link2 className="h-4 w-4" /></button>
      </div>
    </article>
  );
}
...
function UploadPage({ editId }: { editId?: number }) {
  ...
  const upload = trpc.pdfs.upload.useMutation({ onSuccess: result => { toast.success(result.message); setFile(null); setFileData(""); setLocation(`/studentshare/pdf/${result.id}`); }, onError: error => toast.error(error.message) });
...
function AuthPage({ mode }: { mode: "login" | "signup" }) {
  return <AppShell><section className="mx-auto flex max-w-xl flex-col items-center px-5 py-20 text-center lg:py-28"><span className="flex h-16 w-16 items-center justify-center rounded-[20px] bg-[#dfe5ff] text-[#3457e5]"><BookOpenCheck className="h-8 w-8" /></span><p className="mt-7 text-xs font-bold uppercase tracking-[.18em] text-[#3457e5]">StudentShare</p><h1 className="mt-3 font-display text-4xl font-bold tracking-[-.06em] text-slate-950">{mode === "login" ? "Welcome back." : "Join the study circle."}</h1><p className="mt-4 max-w-md text-sm leading-6 text-slate-500">{mode === "login" ? "Log in to upload, save, and keep your study materials in one place." : "Create your student profile to share notes and discover better ways to learn."}</p><button onClick={() => startLogin()} className="mt-8 inline-flex items-center justify-center gap-2 rounded-full bg-[#3457e5] px-7 py-3.5 text-sm font-bold text-white shadow-[0_12px_24px_rgba(52,87,229,0.2)] hover:bg-[#2849d7]">Continue with StudentShare <ArrowDownToLine className="h-4 w-4 rotate-[-45deg]" /></button><p className="mt-5 text-xs text-slate-400">Secure sign-in powered by Manus OAuth.</p><div className="mt-14 grid w-full grid-cols-3 gap-3 text-left"><div className="rounded-2xl bg-white p-4"><ShieldCheck className="h-4 w-4 text-[#3457e5]" /><p className="mt-3 text-xs font-bold text-slate-700">Secure account</p></div><div className="rounded-2xl bg-white p-4"><Share2 className="h-4 w-4 text-[#ed6b3d]" /><p className="mt-3 text-xs font-bold text-slate-700">Share freely</p></div><div className="rounded-2xl bg-white p-4"><Library className="h-4 w-4 text-[#16815e]" /><p className="mt-3 text-xs font-bold text-slate-700">Learn together</p></div></div></section></AppShell>;
}
