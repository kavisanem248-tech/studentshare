  ...
  return (
    <article className={`group rounded-[22px] border border-slate-200/80 bg-white p-4 shadow-[0_10px_30px_rgba(15,23,42,0.04)] transition-all duration-200 hover:-translate-y-1 hover:shadow-[0_18px_38px_rgba(15,23,42,0.09)] ${compact ? "" : "sm:p-5"}`}>
      ...
      <Link href={`/studentshare/pdf/${pdf.id}`} className="mt-4 block">
        <div className="flex items-start gap-2"><div className="line-clamp-2 min-h-[48px] flex-1 font-display text-[17px] font-bold leading-6 tracking-[-0.02em] text-slate-900 group-hover:text-[#3457e5]">{pdf.title}</div>{pdf.groupId && <span className="shrink-0 rounded-full bg-[#fff0e7] px-2 py-1 text-[10px] font-bold text-[#c65a38]">Circle</span>}</div>
...
export function GroupsPage() {
  const { user } = useAuth();
  const { data: groups = [], refetch } = trpc.groups.mine.useQuery(undefined, { enabled: Boolean(user) });
  const [createForm, setCreateForm] = useState({ name: "", password: "" });
  const [joinForm, setJoinForm] = useState({ groupId: "", password: "" });
  const [createdGroup, setCreatedGroup] = useState<{ groupId: string; name: string } | null>(null);
  const create = trpc.groups.create.useMutation({ onSuccess: group => { setCreatedGroup(group); setCreateForm({ name: "", password: "" }); refetch(); toast.success("Friend circle created"); }, onError: error => toast.error(error.message) });
  const join = trpc.groups.join.useMutation({ onSuccess: result => { setJoinForm({ groupId: "", password: "" }); refetch(); toast.success(`Joined ${result.group.name}`); }, onError: error => toast.error(error.message) });
  if (!user) return <AuthPrompt />;
  return <AppShell><div className="mx-auto max-w-[1200px] px-5 py-10 lg:px-10 lg:py-14"><PageTitle eyebrow="Private study spaces" title="Your friend circles" description="Create a password-protected circle for your friends. Only members can see its PDFs." /><div className="mt-9 grid gap-5 lg:grid-cols-2"><form onSubmit={event => { event.preventDefault(); create.mutate(createForm); }} className="rounded-[24px] border border-[#cfd6ff] bg-[#f4f6ff] p-6"><div className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-[14px] bg-white text-[#3457e5] shadow-sm"><Plus className="h-5 w-5" /></span><div><h2 className="font-display text-lg font-bold text-slate-950">Make a friend circle</h2><p className="text-sm text-slate-500">A private space for your study group.</p></div></div><div className="mt-6 grid gap-3"><Input required minLength={2} placeholder="Circle name, e.g. CS study crew" value={createForm.name} onChange={event => setCreateForm({ ...createForm, name: event.target.value })} className="h-11 rounded-xl border-white bg-white" /><Input required minLength={8} type="password" placeholder="Create a password (8+ characters)" value={createForm.password} onChange={event => setCreateForm({ ...createForm, password: event.target.value })} className="h-11 rounded-xl border-white bg-white" /><Button disabled={create.isPending} className="h-11 rounded-xl bg-[#3457e5] font-bold hover:bg-[#2849d7]">{create.isPending ? "Creating..." : "Create circle"}</Button></div>{createdGroup && <div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><p className="text-xs font-bold uppercase tracking-[.14em] text-emerald-700">Share these with your friends</p><p className="mt-2 font-display text-xl font-bold text-slate-950">{createdGroup.groupId}</p><p className="mt-1 text-xs leading-5 text-slate-600">Give friends this group ID and the password you chose. The password is never shown again.</p></div>}</form><form onSubmit={event => { event.preventDefault(); join.mutate(joinForm); }} className="rounded-[24px] border border-slate-200/80 bg-white p-6 shadow-[0_10px_30px_rgba(15,23,42,0.04)]"><div className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-[14px] bg-[#fff0e7] text-[#ed6b3d]"><Users className="h-5 w-5" /></span><div><h2 className="font-display text-lg font-bold text-slate-950">Join a friend circle</h2><p className="text-sm text-slate-500">Use the ID and password shared by a friend.</p></div></div><div className="mt-6 grid gap-3"><Input required placeholder="Group ID, e.g. FRIEND-A1B2C3" value={joinForm.groupId} onChange={event => setJoinForm({ ...joinForm, groupId: event.target.value.toUpperCase() })} className="h-11 rounded-xl border-slate-200 font-mono uppercase" /><Input required type="password" placeholder="Group password" value={joinForm.password} onChange={event => setJoinForm({ ...joinForm, password: event.target.value })} className="h-11 rounded-xl border-slate-200" /><Button disabled={join.isPending} variant="outline" className="h-11 rounded-xl border-slate-200 font-bold">{join.isPending ? "Joining..." : "Join circle"}</Button></div></form></div><div className="mt-10"><div className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-primary">Member access</p><h2 className="mt-2 font-display text-2xl font-bold tracking-[-.04em] text-slate-950">Circles you can access</h2></div><Link href="/upload" className="text-sm font-bold text-[#3457e5] hover:text-[#2849d7]">Share a PDF</Link></div>{groups.length ? <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{groups.map(group => <div key={group.id} className="rounded-[22px] border border-slate-200/80 bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.04)]"><div className="flex items-start justify-between gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-[12px] bg-[#dfe5ff] text-[#3457e5]"><Users className="h-5 w-5" /></span><Badge className="bg-[#eef1ff] text-[#3457e5]">{group.role === "owner" ? "Owner" : "Member"}</Badge></div><h3 className="mt-5 font-display text-lg font-bold text-slate-950">{group.name}</h3><p className="mt-2 font-mono text-sm font-bold tracking-wide text-[#3457e5]">{group.groupId}</p><p className="mt-3 text-sm text-slate-500">{group.memberCount} {group.memberCount === 1 ? "member" : "members"} · private PDFs</p><Link href="/browse" className="mt-5 inline-flex text-sm font-bold text-slate-700 hover:text-[#3457e5]">Browse circle materials <ChevronRight className="ml-1 h-4 w-4" /></Link></div>)}</div> : <div className="mt-5 rounded-[22px] border border-dashed border-slate-300 bg-white p-10 text-center"><Users className="mx-auto h-8 w-8 text-slate-300" /><p className="mt-3 font-bold text-slate-700">No friend circles yet</p><p className="mt-1 text-sm text-slate-500">Create one for your circle or join one with an ID and password.</p></div>}</div></div></AppShell>;
}

function UploadPage({ editId }: { editId?: number }) {
  const { user } = useAuth();
  const { data: subjects = [] } = trpc.subjects.list.useQuery();
  const { data: groups = [] } = trpc.groups.mine.useQuery(undefined, { enabled: Boolean(user) });
  ...
  const [form, setForm] = useState({ title: "", subjectId: "", unit: "Unit 1", description: "", tags: "", academicYear: "2025–26", semester: "", groupId: "" });
  useEffect(() => { if (existing) setForm({ title: existing.title, subjectId: String(existing.subjectId), unit: existing.unit, description: existing.description || "", tags: existing.tags || "", academicYear: existing.academicYear || "", semester: existing.semester || "", groupId: existing.groupId ? String(existing.groupId) : "" }); }, [existing]);
  if (!user) return <AuthPrompt />;
...
function PdfCanvasViewer({ src, title }: { src: string; title: string }) {
    ...
    const loadPdf = async () => {
      ...
      try {
        ...
        for (let pageNumber = 1; pageNumber <= document.numPages; pageNumber += 1) {
          ...
          const width = Math.max(canvasHost.current.clientWidth - 32, 280);
          const scale = Math.min(1.8, width / baseViewport.width);
          const viewport = page.getViewport({ scale });
          const canvas = window.document.createElement("canvas");
          canvas.setAttribute("aria-label", `${title}, page ${pageNumber}`);
          canvas.className = "mx-auto mb-5 block max-w-full rounded-lg bg-white shadow-[0_8px_24px_rgba(15,23,42,0.1)]";
  ...
  return <div className="relative min-h-[680px] bg-[#e9ecf3] p-4 sm:p-6">{status === "loading" && <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 text-center"><Loader2 className="h-8 w-8 animate-spin text-[#3457e5]" /><p className="text-sm font-bold text-slate-600">Loading PDF preview…</p></div>}{status === "error" && <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 px-6 text-center"><FileText className="h-10 w-10 text-slate-300" /><p className="font-bold text-slate-700">Preview could not be rendered.</p><p className="max-w-md text-sm text-slate-500">You can still open the original PDF in a new tab.</p><a href={src} target="_blank" rel="noreferrer" className="rounded-full bg-[#3457e5] px-4 py-2 text-sm font-bold text-white">Open PDF</a></div>}<div ref={canvasHost} className={status === "ready" ? "" : "invisible"} />{status === "ready" && <p className="pb-2 text-center text-xs font-semibold text-slate-400">{pageCount} {pageCount === 1 ? "page" : "pages"} · rendered in StudentShare</p>}</div>;
}
